...
class Base
{
    ...
    public function callback_date($args)
    {
        ...
        $html = sprintf('<input type="date" class="%1$s-text" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>', $size, $args['section'], $args['id'], $value);
        $html .= $this->get_field_description($args);
        ...
        <script>
            jQuery(document).ready(function($) {
                ...
                $('.group .collapsed').each(function() {
                    $(this).find('input:checked').parent().parent().parent().softAll().each(
                        function() {
